...
import {
  ...
  resetPassword,
  updateDynamicUser,
} from "../../services/user_services/user.services";
...
class RegisterController {
  public async register(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (existingUser) {
        res
          .status(400)
          .send({ message: existingUser, status: false, code: 400 });
        return;
      }
      // Hash the password
      ...
      sendEmail(
        ...
        `<a href="${ENV.FRONTEND_URL}/veryfy-email?token=${token}">Verify Email</a>`
      );
  ...
  public async veryficationsEmail(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!token) {
        res
          .status(400)
          .send({ message: ERROR.TOKEN_REQUIRED, status: false, code: 400 });
        return;
      }
      ...
      if (decoded) {
        if (currentTimestamp > decoded.exp) {
          ...
          if (decoded.verifyEmail) {
            newUser = (await updateDynamicUser(decoded.email, {
        ...
        res
          .status(400)
          .send({ message: ERROR.INVALID_TOKEN, status: false, code: 400 });
        return;
      }
      res.status(200).send({
  ...
  public async forgetPassword(req: Request, res: Response): Promise<void> {
    try {
      ...
      const user: UserType | null = (await getUserByEmailOrUsername(
        emailOrUsername as string
      )) as UserType | null;
      if (!user) {
        res
          .status(404)
          .send({ message: ERROR.NOT_FOUND, status: false, code: 404 });
        return;
      }
  ...
  public async loginUser(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (isPasswordValid) {
        ...
        res
          .status(400)
          .send({ message: ERROR.INVALID_PASSWORD, status: false, code: 400 });
        return;
      }
    } catch (error) {
      res.status(500).send({
  ...
  public async logout(
    req: Request & { logout: (callback: (err: any) => void) => void },
    res,
    next
  ): Promise<void> {
    req.logout((err) => {
