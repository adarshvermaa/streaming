  ...
  return new Promise(async (resolve, reject) => {
    try {
      const user = await db
        ...
        .where(
          or(
            eq(UsersModel.username, emailOrUsername),
            eq(UsersModel.email, emailOrUsername)
          )
        )
        .then((result) => result[0]);
      resolve(user);
...
export const resetPassword = async (email: string, password: string) => {
  return new Promise(async (resolve, reject) => {
    try {
      const data = { password, isEmailVerifiedAt: new Date() };
...
export const updateDynamicUser = async (
  email: string,
  data: { [key: string]: any }
) => {
  return new Promise(async (resolve, reject) => {
