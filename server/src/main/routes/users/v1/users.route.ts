...
import {
  ...
  userUpdateSchema,
} from "../../../zod_schemas/users_schemas/user.schemas";
...
UserRouter.post(
  "/forget-password",
  zodValidator(userForgetPasswordSchema),
  registerController.forgetPassword
);
UserRouter.post(
  "/reset-password",
  zodValidator(userResetPasswordSchema),
...
UserRouter.put(
  "/user-update",
  authrization,
  zodValidator(userUpdateSchema),
  registerController.updateUser
);
UserRouter.get("/logout", registerController.logout);
UserRouter.get("/all", authrization, registerController.geAllUsers);
