...
export const WorkspaceMembersModel = pgTable(
  "workspace_members",
  {
    ...
    invitationStatus: invitationStatusEnum("invitation_status")
      .default("pending")
      .notNull(),
    invitedBy: uuid("invited_by").references(() => UsersModel.id, {
      onDelete: "set null",
    }),
  ...
  (table) => ({
    ...
    invitationStatusIdx: index("invitation_status_idx").on(
      table.invitationStatus
    ),
    invitationTokenIdx: index("invitation_token_idx").on(table.invitationToken),
  })
);
